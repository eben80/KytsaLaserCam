...
WebSocketsClient::WebSocketsClient() {
	  _subprotocol = "";  // default no subprotocol
    _cbEvent             = NULL;
    _client.num          = 0;
...
void WebSocketsClient::begin(const char * host, uint16_t port, const char * url, const char * protocol) {
    _host = host;
    _port = port;
	  _subprotocol = String(protocol);  // store the protocol here
#if defined(HAS_SSL)
...
void WebSocketsClient::begin(String host, uint16_t port, String url, String protocol) {
    // If the protocol is empty, pass a null pointer to prevent setting a default
    if (protocol.length() == 0) {
        begin(host.c_str(), port, url.c_str(), nullptr);
    } else {
...
void WebSocketsClient::sendHeader(WSclient_t * client) {
    ...
    if (ws_header) {
        ...
        // ✅ Only add this if protocol string is non-empty
    ...
    // Add any extra headers (e.g., "Origin", etc.)
